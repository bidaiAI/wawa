 ...
 *
 * Connect wallet → auto-detect AIs from Factory contract → manage Twitter etc.
 *
 * Privacy boundaries:
...
function DashboardPage() {
  ...
  // Deduplicate: same vault address on multiple chains → single card
  ...
  return (
    <div className={`bg-[#0d0d0d] border rounded-xl p-5 transition-all hover:border-[#2d3748] ${
      ...
      <div className="mb-3 flex items-center justify-between">
        {apiLoading ? (
          <span className="text-[#4b5563] text-xs">checking twitter...</span>
        ) : twitterConnected ? (
          ...
          <a
            ...
            className="inline-flex items-center gap-1.5 px-4 py-2 rounded-lg text-xs font-bold uppercase tracking-wider
          ...
          <span className="text-[#4b5563] text-xs">twitter unavailable (dead)</span>
        )}
      </div>
